...
class RentService(AbstractService[RentCreate, RentUpdate]):
    ...
    async def create(self, rent_create_dto: RentCreate) -> Rent:

        car = await self.car_repo.get_by_id(rent_create_dto.car_id)
        ...
        user = await self.user_repo.get_by_id(rent_create_dto.client_id)
        if not user or not user.active:
            raise HTTPException(status_code=404, detail="User not found")
        car_upd_dto = CarUpdate(
            available=False,
        )
        await self.car_repo.update(rent_create_dto.car_id, car_upd_dto)
        data = rent_create_dto.model_dump()
        print(type(data["car_id"]))
        rent = Rent(
            **rent_create_dto.model_dump(),
            total_price=rent_create_dto.days_qty * car.price_per_day
        )
